...
// This "Voting Machine" app is based on the original sample app from Microsoft
// https://github.com/Azure/azure-sphere-samples/tree/master/Samples/AzureIoT
// 1. Use the button A to send "Yes" vote
// 2. Use the button B to send "No" vote
// 3. Use IoT Hub/Device Twin to control an LED.
...
// "AzureSphereTargetHardwareDefinitionDirectory" property in CMakeSettings.json
// has been updated to target AvNET Azure Sphere Starter Kit
...
static char scopeId[SCOPEID_LENGTH]; // ScopeId for Azure IoT Hub, set in app_manifest.json, CmdArgs
...
// Buttons
static int sendYesMessageButtonGpioFd = -1;
static int sendNoMessageButtonGpioFd = -1;
...
static GPIO_Value_Type sendYesMessageButtonState = GPIO_Value_High;
static GPIO_Value_Type sendNoMessageButtonState = GPIO_Value_High;

static void ButtonPollTimerEventHandler(EventData *eventData);
static bool IsButtonPressed(int fd, GPIO_Value_Type *oldState);
static void SendYesMessageButtonHandler(void);
static void SendNoMessageButtonHandler(void);
...
static void ButtonPollTimerEventHandler(EventData *eventData)
{
    ...
    SendYesMessageButtonHandler();
    SendNoMessageButtonHandler();
}
...
static int InitPeripheralsAndHandlers(void)
{
    ...
    sendYesMessageButtonGpioFd = GPIO_OpenAsInput(SAMPLE_BUTTON_1);
    if (sendYesMessageButtonGpioFd < 0) {
    ...
    sendNoMessageButtonGpioFd = GPIO_OpenAsInput(SAMPLE_BUTTON_2);
    if (sendNoMessageButtonGpioFd < 0) {
...
static void ClosePeripheralsAndHandlers(void)
{
    ...
    CloseFdAndPrintError(sendYesMessageButtonGpioFd, "SendYesMessageButton");
    CloseFdAndPrintError(sendNoMessageButtonGpioFd, "SendNoMessageButton");
...
///     Send a 'Button Pressed' event to Azure IoT Hub
/// </summary>
static void SendYesMessageButtonHandler(void)
{
    if (IsButtonPressed(sendYesMessageButtonGpioFd, &sendYesMessageButtonState)) {
        SendTelemetry("Voted", "Yes");
    }
}

/// <summary>
/// Pressing button B will:
///     Send an 'Button Pressed' event to Azure IoT Hub
/// </summary>
static void SendNoMessageButtonHandler(void)
{
    if (IsButtonPressed(sendNoMessageButtonGpioFd, &sendNoMessageButtonState)) {
		SendTelemetry("Voted", "No");;
    }
}
